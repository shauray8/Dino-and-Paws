...
class DINOHead(nn.Module):
    def __init__(self, in_dim, out_dim, use_bn=False, norm__last_layer=True, 
        ...
        if nlayers == 1:
            ...
            for _ in range(nlayers - 2):
                layers.append(nn.Linear(hidden_dim, hiidden_dim))
                if use_bn:
                    layers.append(nn.BatchNorm1d(hidden_dim))
                layers.append(nn.GELU())
        ...
        self.apply(self._init_weights)
        self.last_layer = nn.utils.weight_norm(n.Linear(bottleneck_dim, out_dim, bias=False))
